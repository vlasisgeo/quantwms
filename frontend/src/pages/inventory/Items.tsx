...
export default function Items() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...d }: Partial<Item> & { id: number }) => itemsApi.update(id, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['items'] }); setModal(null) },
  })
  ...
  function openEdit(item: Item) {
    reset({ sku: item.sku, name: item.name, description: item.description, weight_grams: item.weight_grams, active: item.active })
    setModal(item)
  }
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input label="SKU" id="sku" error={errors.sku?.message} {...register('sku')} disabled={!!isEdit} />
